...
DEPENDS += "${@bb.utils.contains('USERDATA_FSTYPE', 'f2fs', 'f2fs-tools-native f2fs-tools-android-native', '', d)} "
...
do_makeuserdata() {
    ...
    if ${@bb.utils.contains('USERDATA_FSTYPE', 'f2fs', 'true', 'false', d)}; then
        ...
        dd if=${IMAGE_ROOTFS}/${USERDATA_DIR} of=${IMGDEPLOYDIR}/${IMAGE_BASENAME}/${USERDATAIMAGE_TARGET} seek=${USERDATA_IMAGE_ROOTFS_SIZE} count=0 bs=1
        make_f2fs -O encrypt -S ${USERDATA_IMAGE_ROOTFS_SIZE} -f -R  root:root ${IMGDEPLOYDIR}/${IMAGE_BASENAME}/${USERDATAIMAGE_TARGET}
    else
        ...
        make_ext4fs -C ${WORKDIR}/rootfs-fsconfig.conf \
                    ...
                    -S -b 4096 -l ${USERDATA_IMAGE_ROOTFS_SIZE} \
                    ${IMGDEPLOYDIR}/${IMAGE_BASENAME}/${USERDATAIMAGE_TARGET}.unsparsed \
