...
CROND_TABS_DIR = "${sysconfdir}"
SYSTEMD_PACKAGES = "${PN}-crond"
SYSTEMD_SERVICE_${PN}-crond = "busybox-crond.service"

RDEPENDS_${PN}-crond = "busybox"

FILES_${PN} += "/usr/bin/env"
FILES_${PN}-crond = "{CROND_TABS_DIR}/crontabs ${systemd_unitdir}/system/busybox-crond.service"
...
do_configure_append() {
    ...
    sed -i 's/# CONFIG_CROND is not set/CONFIG_CROND=y/' ${S}/.config
    sed -i 's/# CONFIG_FEATURE_CROND_D is not set/CONFIG_FEATURE_CROND_D=y/' ${S}/.config
    sed -i 's/# CONFIG_FEATURE_CROND_CALL_SENDMAIL is not set/CONFIG_FEATURE_CROND_CALL_SENDMAIL=y/' ${S}/.config
    sed -i 's@CONFIG_FEATURE_CROND_DIR="\/var\/spool\/cron"@'"CONFIG_FEATURE_CROND_DIR=\"${CROND_TABS_DIR}\""'@' ${S}/.config
    sed -i 's/# CONFIG_CRONTAB is not set/CONFIG_CRONTAB=y/' ${S}/.config
}
...
do_install_append() {
    ...
    if ${@bb.utils.contains('DISTRO_FEATURES','systemd','true','false',d)}; then
        ...
        if grep -q "CONFIG_CROND=y" ${B}/.config; then
            install -d ${D}${CROND_TABS_DIR}/crontabs
            install -d ${D}${systemd_unitdir}/system
            install -m 755 ${WORKDIR}/busybox-crond.service ${D}${systemd_unitdir}/system/busybox-crond.service
        fi
    else
