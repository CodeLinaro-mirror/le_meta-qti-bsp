...
get_gvm_version() {
    ...
    system_name_value=$(echo "$cmdline_value" | sed -n 's/.*system_name=\([^ ]*\).*/\1/p')
...
check_all_interfaces_up() {
    iface_arr="$1"
    for iface_name in $iface_arr; do
        iface_cnt=$(ifconfig -a | grep "$iface_name" | wc -l)
        if [ "$iface_cnt" -ne 1 ]; then
