...
SRC_URI = "\
    ...
    file://setup-network-host-lxc.sh \
    file://setup-network-host-lxc.service \
"
...
do_install:append:lemans() {
  ...
  if ${@bb.utils.contains('DISTRO', 'auto-lxc', 'true', 'false', d)}; then
      install -m 0755 ${WORKDIR}/setup-network-host-lxc.sh ${D}${bindir}/setup-network-host.sh
      install -m 0644 ${WORKDIR}/setup-network-host-lxc.service ${D}${systemd_unitdir}/system/setup-network-host.service
  else
...
SYSTEMD_SERVICE:${PN}:lemans = "setup-network-host.service"
