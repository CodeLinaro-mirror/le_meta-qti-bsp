   ...
   for ubidev in /dev/ubi[1-99]_*; do
      ...
      if [ "$volname" == "$volume" ]; then
         ...
         if [ "$fstab_only" != "1" ]; then
	        ...
	        echo "MTD : Mounting of $ubidev failed on $dir" > /dev/kmsg
	    fi
         fi
...
FindAndMountEXT4 () {
   ...
   if [ "$fstab_only" != "1" ]; then
      ...
      if [ $? -eq 0 ]; then
          ...
          echo "EMMC : Mounting of $mmc_block_device failed on $dir"
      fi
   fi
...
FindAndMountMTD () {
   ...
   if [ $? -eq 0 ]; then
       ...
       echo "Mounting of /dev/$mmc_block_device failed on $dir" > /dev/kmsg
   fi
