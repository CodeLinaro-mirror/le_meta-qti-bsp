...
do_install() {
    # Install modules
    mkdir -p ${D}/usr/lib/modules/${KERNEL_VERSION}
    for mod in *.ko; do
        if [ -f $mod ]; then
            install -m 0644 $mod ${D}/usr/lib/modules/${KERNEL_VERSION}
        fi
    done
    # Create empty modules.load as a place holder to mimic Android GKI ramdisk
    touch ${D}/usr/lib/modules/${KERNEL_VERSION}/modules.load
...
FILES:${PN}-dbg += "/usr/lib/modules/${KERNEL_VERSION}/.debug"
...
python get_files_pn_from_conf() {
    ...
    f_mods = [ '/etc/modules-load.d/00-firstmods.conf', '/usr/lib/modules/*/modules.load' ]
    with open(f_conf) as f:
        lines = f.readlines()
        for line in lines:
            if line.startswith('#'):
                continue
            f_mods += [ '/usr/lib/modules/*/' + line.rstrip() + '.ko' ]
    ...
    with open(s_conf) as f:
        ...
        for line in lines:
            if line.startswith('#'):
                continue
            s_mods += [ '/usr/lib/modules/*/' + line.rstrip() + '.ko' ]
