...
do_shared_workdir_append () {
        ...
        install -m 0644 vmlinux ${DEPLOY_DIR_IMAGE}/vmlinux
...
do_install_append () {
    # Rename kernel images if MACHINE_SUPPORTS_NAND_RECOVERY is set to True
    if [ ! -z "${MACHINE_SUPPORTS_NAND_RECOVERY}" ]; then
        if [ -f "${DEPLOY_DIR_IMAGE}/${KERNEL_IMAGETYPE}" ]; then
            mv ${DEPLOY_DIR_IMAGE}/${KERNEL_IMAGETYPE} ${DEPLOY_DIR_IMAGE}/${KERNEL_IMAGETYPE}${KERNEL_IMAGENAME}
        fi

        if [ -f "${DEPLOY_DIR_IMAGE}/vmlinux" ]; then
            mv ${DEPLOY_DIR_IMAGE}/vmlinux ${DEPLOY_DIR_IMAGE}/vmlinux${KERNEL_IMAGENAME}
        fi
    fi
}
